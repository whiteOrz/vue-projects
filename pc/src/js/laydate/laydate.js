...
!function(a) {
	...
	c.view = function(a, f) {
		...
		function() {
			...
			a
		} () + "</div>" + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">时间</li>' + "<li><input readonly>:</li>" + "<li><input readonly>:</li>" + "<li><input readonly></li>" + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<p id="laydate_today" style="display:none;float:left"></p>' + '<a id="laydate_ok">确认</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>": "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null),
		c.follow(c.box),
	...
	c.events = function() {
		...
		c.each(b("#laydate_table td"),function(a, b) {
			c.on(b, "click",function(a) {
				...
				if (window.laydate_table_event) {
					window.laydate_table_event();
				}
			})
		}),
