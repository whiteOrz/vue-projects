...
var checkFn = {
			...
			if($.trim(mobile) == "" || $.trim(mobile).length < mobile.length) {
				...
				if(mobile.length != 11) {
					...
					if(!re.test(mobile)) {
						...
						if((Math.abs(o) == 1 && obj[1])||(Math.abs(o) == 1 && obj[-1])) {
							return {
								onOff: false,
								message: "手机号码格式不对，请正确填写！"
							}
	...
	"validateCid": function(cid, val, minAge, maxAge, type) {
		if(cid == "") {
			return {
				onOff: false,
				message: "请选择证件类型"
		...
		if(cid == "01") {
			...
			if(type) {
				//投保人
				if(maxAge >= 200) {
					var errorMessage = "投保人年龄应大于" + minAge + "周岁";
				} else {
					var errorMessage = "投保人年龄应在" + minAge + "到" + maxAge + "周岁之间";
				}
				if(!checkFn.birthRange(val, minAge, maxAge)) {
					return {
						onOff: false,
				...
				var errorMessage = "被保人年龄应在" + minAge + "到" + maxAge + "之间";

				if(!checkFn.checkInsuredAge(val, minAge, maxAge)) {
					return {
						onOff: false,
						message: errorMessage
					}
	...
	//投保人年龄  maxAge200以上表明只是成年人即可
	"birthRange": function(val, minAge, maxAge) {
		var min = parseInt(minAge);
		var max = parseInt(maxAge) + 1;
		var birthday, Y, M, D;
		if(val.length == 15) {
			Y = "19" + val.substr(6, 2);
			M = val.substr(8, 2) - 1;
	...
	"checkInsuredAge": function(val, minAge, maxAge) {
		if(minAge.indexOf("天") != -1) {
			var day = parseInt(minAge);
			min = 0;
		} else {
			min = parseInt(minAge);
		}
		if(maxAge.indexOf("岁") != -1) {
			max = parseInt(maxAge) + 1;
		}else{
			max = parseInt(maxAge) + 1;
		}
		...
		if(offsetYear < min) {
			...
			var daySecond=parseInt(day*24*60*60*1000);
			if(now.getTime()-birth.getTime()<daySecond){
				return false
			}
			if(birthMonth > month) {
				//fail
	...
	//受益人证件号码
	"validateBenCid": function(cid, val) {
		if(cid == "") {
			return {
				onOff: false,
				message: "请选择证件类型"
