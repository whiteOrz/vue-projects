        ...
        for (i = 0, il = json.length; i < il; i += 1) {
            ...
            switch (currentChar) {
                ...
                case ',':
                    ...
                    break;
                case ':':
                    if (!inString) { newJson += ": " } else { newJson += currentChar }
                    break;
                case ' ':
                case "\n":
                case "\t":
...
jsl.interactions = (function() {
    ...
    function validate() {
        ...
        try {
            ...
            if (result) {
                ...
                if (compress) { $('#json_input').val(JSON.stringify(JSON.parse(jsonVal), null, "")) }
            } else { alert("An unknown error occurred. Please contact Arc90.") }
            ...
            if (lineMatches && typeof lineMatches === "object" && lineMatches.length > 1) {
                ...
                if (lineEnd < 0) { lineEnd = jsonVal.length }
    ...
    function init() {
        ...
        if (compress) { $('#headerText').html('JSONLint<span class="light">Compressor</span>') }
        if (!reformat) { $('#headerText').html('JSONLint<span class="light">Lite</span>') }
        ...
        $('#json_input').keyup(function() { $('div.linedwrap').removeClass('greenBorder').removeClass('redBorder') }).linedtextarea({ selectedClass: 'lineselect' }).focus();
...
$(function() { jsl.interactions.init() });
