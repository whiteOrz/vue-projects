...
var phContaine = {
	...
	"phNameContainer": function(opt) {
		...
		if(!ordcheckData.onOff) {
			...
			if(opt.subTip) {
				layer.msg('投保人' + ordcheckData.message);
	...
	"phCidNumContainer": function(opt) {
		...
		var tip = '';
		//校验合法性  身份证的合法性
		var checkedData = checkFn.validateCid(cidtype_id, opt.value);
		if(!checkedData.onOff) {
			
			tip = opt.value ? '投保人' + checkedData.message : '投保人证件号码不能为空';
			phContaine.checkTipInfor(opt.selector, checkedData.message, checkedData.onOff, opt.mark);
...
//			subTip: false
	"phBirthdayContainer": function(opt) {
		...
		if(opt.value=="") {
			phContaine.checkTipInfor(opt.selector, '请选择投保人出生日期', false, opt.mark);
			if(opt.subTip) {
				layer.msg('请选择投保人出生日期');
				phContaine.subScrollTop($(opt.selector));
			}
			return false;
		}else{
